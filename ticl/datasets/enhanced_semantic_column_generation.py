...
class EnhancedColumnSemanticTokenizer(ColumnSemanticTokenizer):
    ...
    def generate_statistical_causal_relationships(self, column_name: str, value_ranges: Optional[List[str]] = None):
        """
        Generate statistical causal relationships for a column.
        
        Args:
            column_name: The name of the column
            value_ranges: Optional list of value ranges for the column (e.g., ["low", "medium", "high"])
            
        Returns:
            Generated causal relationships as a Python dictionary
        """
        # Default value ranges if none provided
        if value_ranges is None or not value_ranges:
            value_ranges = [
                f"low {column_name}", 
                f"medium {column_name}", 
                f"high {column_name}"
            ]
            
        # Join the value ranges for the prompt
        value_ranges_text = "\n".join([f"- {value_range}" for value_range in value_ranges])
        
        # Safely format the prompt template to avoid issues with special characters in column names
        try:
            # Use a safe format with properly escaped parameters
            prompt = PROMPT_TEMPLATES['statistical_causal_relationships'].format(
                column_name=column_name,
                value_ranges=value_ranges_text
            )
        except KeyError as e:
            # If there's a KeyError, it could be due to special characters in column_name or value_ranges
            # Create safer versions by escaping curly braces
            safe_column_name = str(column_name).replace("{", "{{").replace("}", "}}")
            safe_value_ranges = str(value_ranges_text).replace("{", "{{").replace("}", "}}")
            
            # Try again with the safer strings
            prompt = PROMPT_TEMPLATES['statistical_causal_relationships'].format(
                column_name=safe_column_name,
                value_ranges=safe_value_ranges
            )
        ...
        try:
            ...
            # Parse the response to extract the Python dictionary
            # Clean the text to prepare for extraction
            clean_text = response.replace('```python', '').replace('```', '')
            ...
            if start_idx != -1 and end_idx != -1 and end_idx > start_idx:
                ...
                try:
                    ...
                    # If parsing fails, return a simple fallback dictionary
                    print(f"Error parsing response: {e}")
                    return self._generate_fallback_causal_relationships(column_name, value_ranges)
            else:
                return self._generate_fallback_causal_relationships(column_name, value_ranges)
                
        except Exception as e:
            print(f"Error generating causal relationships: {e}")
            return self._generate_fallback_causal_relationships(column_name, value_ranges)
            
    def _generate_fallback_causal_relationships(self, column_name: str, value_ranges: List[str]):
        """
        Generate fallback causal relationships when the LLM fails.
        
        Args:
            column_name: The name of the column
            value_ranges: List of value ranges for the column
            
        Returns:
            A simple fallback dictionary of causal relationships
        """
        result = {}
        for value_range in value_ranges:
            result[value_range] = [
                {
                    "class": f"{value_range}_category",
                    "causal_explanation": f"Direct categorization based on {value_range} value."
                },
                {
                    "class": f"derived_from_{value_range}",
                    "causal_explanation": f"Derived statistical feature from {value_range} range."
                }
            ]
        return result
